...
def extract_doc_sentences(doc, sent_tokenizer_pattern, dedup_set, multi_word_proper_nouns, params):
        ...
        if (len(doc['sentences']) != 0):
            for sent in doc['sentences']:
                ...
                if (tok_len > params['corenlp_max_sentence_words']):
                    ...
                    filtered_sentences += regex_get_sentences(sent['sentence'], sent_tokenizer_pattern, dedup_set,
                                                              params['corenlp_max_sentence_words'])
                else:
    ...
    if (len(filtered_sentences) == 0):
        filtered_sentences = regex_get_sentences(doc['text'], sent_tokenizer_pattern, dedup_set,
                                                 params['corenlp_max_sentence_words'])
...
def get_ranked_docs(ngram_lst, doc_dct_lst):
    ...
    for i in range(len(ngram_lst)):
        ...
        for posting in ngram_lst[i]['postings']:
            ...
            ranked_docs.setdefault(doc_indx, {'score': 0, 'doc_id': doc_dct_lst[doc_indx]['doc_id'],
...
def rank_sents_frm_top_ranked_docs(ngram_sentences, ranked_docs, all_doc_sentences, extra_params=None):
    ...
    for doc in ranked_docs:
        doc_indx = doc[0]
        doc_id = doc[1]['doc_id']
        all_top_ranked_docs_sentences += get_docs_sentence_score(ngram_sentences, all_doc_sentences[doc_indx], doc_indx,
                                                                 doc_id, extra_params)

    return sorted(all_top_ranked_docs_sentences, key=lambda x: x['avg_overlap'], reverse=True)[
           :extra_params['sentences_rank_count']]
...
def rank_proper_nouns(multi_word_proper_nouns):
    multi_word_proper_nouns = sorted(multi_word_proper_nouns.items(),
                                     key=lambda prnoun: prnoun[1]['freq'] * prnoun[1]['nnp_rate'], reverse=True)
...
def rank_mltwd_proper_nouns(ngram, ngram_toks, sentences, params=None):
    ...
    while window_size <= max_window_size:
        ...
        for i in range(sent_count):
            ...
            for lrb in ['left', 'right', 'both']:
                ...
                if (multi_word_proper_noun_lrb != ngram):
                    ...
                    phrase_counts[lrb][multi_word_proper_noun_lrb]['rate'] = round(
                        phrase_counts[lrb][multi_word_proper_noun_lrb]['freq'] / sent_count, 4)
                    ...
                    logger.debug('\t\t' + lrb + ': ' + multi_word_proper_noun_lrb + ', freq: ' + str(
                        phrase_counts[lrb][multi_word_proper_noun_lrb]['freq']))
        ...
        if (params['mvg_window_min_proper_noun_rate'] > max_multiprpnoun_lrb[window_size][
            'rate'] or window_size == max_sent_toks):
            logger.debug(
                "\tbreaking criteria reached: mvg_window_min_proper_noun_rate > max_multiprpnoun_lrb[window_size]['rate'] OR window_size (" + str(
                    window_size) + ") == max_sent_toks (" + str(max_sent_toks) + ")")
            logger.debug('\tmvg_window_min_proper_noun_rate: ' + str(params['mvg_window_min_proper_noun_rate']))
            logger.debug(
                "\tmax_multiprpnoun_lrb[window_size]['rate']: " + str(max_multiprpnoun_lrb[window_size]['rate']))
            break
    ...
    for i in range(len(top_ngrams)):
        ...
        for mult_wd_prpnoun in ranked_multi_word_proper_nouns:
            ...
            if (match_flag):
                ...
                if (ngram == multi_word_proper_noun or mult_wd_prpnoun[1]['freq'] < top_ngrams[i][
                    'term_freq'] * pos_glue_split_ngrams_coeff):
...
def mvg_window_glue_split_ngrams(top_ngrams, k, all_doc_sentences, params=None):
    ...
    for i in range(len(top_ngrams)):
        ...
        ngram_toks = phraseTokenizer(
            ngram)  # processed in a similar method as the sentences in get_sentence_match_ngram
        ...
        for child_indx in range(len(ngram_tok_sizes)):
            ...
            if (is_ngram_subset(parent_ngram_cand, child_ngram_cand, stopwords)):
                ...
                if (parent_indx < child_indx):
                    ...
                    if (top_ngrams[parent_indx]['term_freq'] >= top_ngrams[child_indx][
                        'term_freq'] * rm_subset_top_ngrams_coeff):
                        ...
                        if (top_ngrams[parent_indx]['adopted_child'] == False):
                            ...
                            logger.debug(
                                '\teven though parent (' + str(parent_indx) + ') is in lower index than child: ' + str(
                                    child_indx))
                            logger.debug(
                                '\treplacing child_ngram_cand:' + '"' + child_ngram_cand + '" with parent_ngram_cand: "' + parent_ngram_cand + '"')
                            logger.debug(
                                '\treplacing parent/child tf: ' + str(top_ngrams[parent_indx]['term_freq']) + ' ' + str(
                                    top_ngrams[child_indx]['term_freq']))
                            logger.debug('')
                        else:
                            ...
                            logger.debug(
                                '\teven though parent (' + str(parent_indx) + ') is in lower index than child: ' + str(
                                    child_indx))
                            logger.debug(
                                '\twould have replaced child_ngram_cand: "' + child_ngram_cand + '" with parent_ngram_cand: "' + parent_ngram_cand + '"')
                            logger.debug('\twould have replaced parent/child tf:' + str(
                                top_ngrams[parent_indx]['term_freq']) + '/' + str(top_ngrams[child_indx]['term_freq']))
...
def print_top_ngrams(n, top_ngrams, top_sumgram_count, params=None):
    ...
    if (params['base_ngram_ansi_color'] == ''):
        print('{:^6} {:<{mw}} {:^6} {:<7} {:<30}'.format('rank', 'sumgram', tf_or_df, tf_or_df + '-Rate', 'Base ngram',
                                                         mw=mw))
    else:
        print('{:^6} {:<{mw}} {:^6} {:<7} {:<30}'.format('rank', getColorTxt('sumgram', default_color), tf_or_df,
                                                         tf_or_df + '-Rate', 'Base ngram', mw=mw))
    ...
    for i in range(top_sumgram_count):
        ...
        print("{:^6} {:<{mw}} {:^6} {:^7} {:<30}".format(i + 1, ngram_txt, ngram['term_freq'],
                                                         "{:.2f}".format(ngram['term_rate']), base_ngram, mw=mw))

    if (len(last_ngram) != 0):
        if (params['min_df'] != 1):
            print('last ngram with min_df = ' + str(
                params['min_df']) + ' (index/' + tf_or_df + '/' + tf_or_df + '-Rate): ' + last_ngram[
                      'ngram'] + ' (' + str(last_ngram['rank']) + '/' + str(last_ngram['term_freq']) + '/' + str(
                last_ngram['term_rate']) + ')')
...
def extract_top_ngrams(doc_lst, doc_dct_lst, n, params):
    ...
    count_vectorizer = CountVectorizer(stop_words=stopwords, token_pattern=params['token_pattern'], ngram_range=(n, n),
                                       binary=binary_tf_flag, min_df=min_df)
...
def update_doc_indx(report, doc_id_new_doc_indx_map):
    ...
    for i in range(len(report['top_sumgrams'])):
        if 'parent_sentences' in report['top_sumgrams'][i]:
...
def get_top_sumgrams(nlp_docs, n=2, params=None):
    ...
    if (params['no_pos_glue_split_ngrams'] == False):
        pos_glue_split_ngrams(top_ngrams, params['top_sumgram_count'] * 2, params['pos_glue_split_ngrams_coeff'],
                              multi_word_proper_nouns, params)
    ...
    top_ngrams = rm_subset_top_ngrams(top_ngrams, params['top_sumgram_count'] * 2, params['rm_subset_top_ngrams_coeff'],
                                      params)
    if (params['print_details'] is True):
    ...
    if (params['no_rank_docs'] == False):
        ...
        if (params['sentences_rank_count'] > 0 and params['no_rank_sentences'] == False):
            ...
            report['ranked_sentences'] = rank_sents_frm_top_ranked_docs(ngram_sentences, report['ranked_docs'],
                                                                        all_doc_sentences, params)
    ...
    fmt_report(report['top_sumgrams'],
               params)  # fmt_report() need to be called last since it potentially could modify merged_ngrams
...
def get_args():
    ...
    parser.add_argument('-n', '--base-ngram',
                        help='The base n (integer) for generating top sumgrams, if n = 2, bigrams would be the base ngram',
                        type=int, default=2)
    parser.add_argument('-m', '--max-ngram', help='The maximum length of sumgram generated', type=int, default=10)
    parser.add_argument('-o', '--output', help='Output file')
    parser.add_argument('-s', '--sentences-rank-count', help='The count of top ranked sentences to generate', type=int,
                        default=10)
    parser.add_argument('-t', '--top-sumgram-count', help='The count of top sumgrams to generate', type=int, default=10)

    parser.add_argument('--add-stopwords',
                        help='Comma-separated list of additional stopwords. To change delimiter use --stopwords-sep',
                        default='')
    parser.add_argument('--collocations-pattern',
                        help='User-defined regex rule to extract collocations for pos_glue_split_ngrams', default='')
    parser.add_argument('--corenlp-max-sentence-words', help='Stanford CoreNLP maximum words per sentence', default=100)
    parser.add_argument('--max-file-depth',
                        help='When reading files recursively from directory stop at the specified path depth. 0 means no restriction',
                        type=int, default=1)
    parser.add_argument('--include-postings', help='Include inverted index of term document mappings',
                        action='store_true')  # default is false except not included, in which case it's true

    parser.add_argument('--log-file', help='Log output filename', default='')
    parser.add_argument('--log-format',
                        help='Log print format, see: https://docs.python.org/3/howto/logging-cookbook.html', default='')
    parser.add_argument('--log-level', help='Log level',
                        choices=['critical', 'error', 'warning', 'info', 'debug', 'notset'], default='info')

    parser.add_argument('--mvg-window-min-proper-noun-rate',
                        help='Mininum rate threshold (larger, stricter) to consider a multi-word proper noun a candidate to replace an ngram',
                        type=float, default=0.5)
    parser.add_argument('--min-df',
                        help='See min_df in https://scikit-learn.org/stable/modules/generated/sklearn.feature_extraction.text.CountVectorizer.html',
                        default=0.01)
    parser.add_argument('--ngram-printing-mw', help='Mininum width for printing ngrams', type=int, default=50)

    parser.add_argument('--base-ngram-ansi-color',
                        help='Highlight (color code format - XXm, e.g., 91m) base ngram when printing top ngrams, set to empty string to switch off color',
                        default='91m')
    parser.add_argument('--no-mvg-window-glue-split-ngrams',
                        help='Do not glue split top ngrams with Moving Window method (default is False)',
                        action='store_true')
    parser.add_argument('--no-parent-sentences',
                        help='Do not include sentences that mention top ngrams in top ngrams payload (default is False)',
                        action='store_true')
    parser.add_argument('--no-pos-glue-split-ngrams',
                        help='Do not glue split top ngrams with POS method (default is False)', action='store_true')
    parser.add_argument('--no-rank-sentences', help='Do not rank sentences flag (default is False)',
                        action='store_true')
    ...
    parser.add_argument('--pos-glue-split-ngrams-coeff',
                        help='Coeff. ([0, 1]) for permitting matched ngram replacement by pos_glue_split_ngrams(), bigger means stricter',
                        type=float, default=0.5)
    parser.add_argument('--pretty-print', help='Pretty print JSON output', action='store_true')
    parser.add_argument('--rm-subset-top-ngrams-coeff',
                        help='Coeff. ([0, 1]) for permitting matched ngram replacement by rm_subset_top_ngrams(), bigger means stricter',
                        type=float, default=0.5)

    parser.add_argument('--sentence-pattern',
                        help='For sentence ranking: Regex string that specifies tokens for sentence tokenization',
                        default='[.?!][ \n]|\n+')
    parser.add_argument('--sentence-tokenizer', help='For sentence ranking: Method for segmenting sentences',
                        choices=['ssplit', 'regex'], default='ssplit')
    ...
    parser.add_argument('--token-pattern', help='Regex string that specifies tokens for document tokenization',
                        default=r'(?u)\b[a-zA-Z\'\’-]+[a-zA-Z]+\b|\d+[.,]?\d*')
    ...
    parser.add_argument('--thread-count',
                        help='Maximum number of threads to use for parallel operations like segmenting sentences',
                        type=int, default=5)
    parser.add_argument('--update-rate', help='Print 1 message per update-rate for long-running tasks', type=int,
                        default=50)
...
#     from util import change_format
